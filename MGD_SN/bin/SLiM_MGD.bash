...
printf "sim_id\toutcome\tNeA\tNeB\ts\tr\tmu\tL\tm\tmut_pos\trise_gen\tfix_gen\tend_gen\tsamp\tmut_freq\trun_CPU_time(s)\tchg_r\tsplit_gen\tseed\tinitSeed\n" >> summary.txt
printf "sim_id\toutcome\tNeA\tNeB\ts\tr\tmu\tL\tm\tmut_pos\trise_gen\tloss_gen\tend_gen\tsamp\tmut_freq\trun_CPU_time(s)\tchg_r\tsplit_gen\tseed\tinitSeed\n" >> log.txt
...
# Step 1 : run SLiM simulations using prior.txt parameters
    ...
    do
        ...
        slim -t -m -d sim_id=${id} -d Ne=${Ne} -d r=${r} -d mu=${mu} -d L=$((${L}-1)) -d samp=${samp} -d chg_r=${chg_r} -d debug=F ../../../bin/MGD-SIM.slim > /dev/null
    done
