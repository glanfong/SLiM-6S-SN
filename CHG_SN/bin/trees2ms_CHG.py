...
def trees2ms(id):
    ...
    ##################################################################################################
    # NO SPLIT NOR MIGRATION IN THIS MODEL - ONLY ONE POPULATION WHICH UNDERGOES A DEMOGRAPHIC EVENT #
    ##################################################################################################
    ...
    # Happens quite early in backward time #
    # chg_gen = np.random.randint(round(0.2*split_gen), round(0.5*split_gen)) -> Define in .slim script and used here as argument !
    ...
    if selection == "sweep":
        recap_mut_tree.dump("./" + str(id) + "_sweep_recap_mut.trees") # save the recapitated & mutated trees
    elif selection == "neutral":
        recap_mut_tree.dump("./" + str(id) + "_neutral_recap_mut.trees") # save the recapitated & mutated trees
