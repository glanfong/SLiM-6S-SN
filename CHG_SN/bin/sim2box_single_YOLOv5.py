...
help_message += '\tto run this script for instance on the simulation producing the following files: 123_parameters.txt 123_positions.txt 123_sumStats.txt 123.ms\n'
...
def readTrees(simulation_target):
	res = {}
	infile = open('{0}.ms'.format(simulation_target), 'r')
	test = 0
	for line in infile:
...
def getSelectedPosition(simulations, L):
	res = {}
	for iteration in range(len(simulations)):
		...
		for line in infile:
			if 'iteration' not in line:
				...
				res[simulations[iteration]] = float(line[7])/L
		infile.close()
	return(res)
...
simulations = [ int(i.split('.')[0]) for i in os.listdir('./') if '.ms' in i ]
simulations.sort()
...
if object_to_recognize == 1:
	selected_positions = getSelectedPosition(simulations=simulations, L=L)
