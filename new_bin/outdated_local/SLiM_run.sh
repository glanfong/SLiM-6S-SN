...
while IFS=$'\t' read -r line; do
    ...
    IFS=$'\t' read -r id Ne_anc NeA r mu L samp chg_r split_gen chg_gen mig_gen SLiM_gen m m_chg <<< "$line"
    IFS=$'\t' read -r id Ne_anc NeA r mu L samp chg_r split_gen chg_gen mig_gen SLiM_gen m m_chg <<< "$line"
    ...
    while [ ! -f "$OUTPUT_DIR/${id}_sweep_parameters.txt" ]; do
        ...
        slim -t -m  -d "OUTPUT_DIR='$OUTPUT_DIR'" -d "sim_id='$id'" -d selection="sweep"-d Ne="$Ne_anc" -d NeA="$NeA" -d r="$r" -d mu="$mu" -d L=$((${L} - 1)) -d samp="$samp" -d chg_r="$chg_r" -d m="$m" -d m_chg="$m_chg" -d SLiM_gen="$SLiM_gen" -d mig_gen="$mig_gen" -d chg_gen="$chg_gen" -d split_gen="$split_gen" -d debug=F ./6S_Sweep-SIM.slim
        
        echo -e " - NEW SLiM INSTANCE - NEUTRAL - $id -"
        slim -t -m  -d "OUTPUT_DIR='$OUTPUT_DIR'" -d "sim_id='$id'" -d selection="neutral"-d Ne="$Ne_anc" -d NeA="$NeA" -d r="$r" -d mu="$mu" -d L=$((${L} - 1)) -d samp="$samp" -d chg_r="$chg_r" -d m="$m" -d m_chg="$m_chg" -d SLiM_gen="$SLiM_gen" -d mig_gen="$mig_gen" -d chg_gen="$chg_gen" -d split_gen="$split_gen" -d debug=F ./6S_Sweep-SIM.slim
